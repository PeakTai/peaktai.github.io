<template>
  <blog-layout>
    <p>
      开发中常常有做统计图的需求，主流的前端插件都是基于 canvas 来做的， 那么使用 css3
      能不能实现呢？今天我来尝试一下使用 css3 做一个饼图效果。
    </p>
    <h3>渐变背景</h3>
    <p>
    ...
    <p>
      参考文档：<a
        ...
        >conic-gradient() - CSS: Cascading Style Sheets | MDN</a
      >。
    </p>
    <h3>元素形状剪裁</h3>
    <p>
      既然要给不同的扇区添加动态效果，必须得让每个扇都是一个元素，然后给元素添加事件或添加伪类样式。那么怎么样才可以让元素显示成扇形的样子呢？这个我们可以通过
      clip-path 来实现，clip-path
      使用裁剪方式创建元素的可显示区域，区域内的部分显示，区域外的隐藏。clip-path
      支持四种图形裁剪：inset，circle，polygon，path，这里我们使用 path
      来直接裁剪出扇形。通过圆角将元素变成圆形，然后再使用多边形（polygon）裁剪也可以得到扇形，但是会复杂非常多。关于
      clip-path 的详细信息，可参考文档：
      <a target="_blank" href="https://developer.mozilla.org/zh-CN/docs/Web/CSS/clip-path"
        >clip-path - CSS（层叠样式表） | MDN</a
      >。
    </p>
    <h4>准备数据</h4>
    <p>
    ...
    <h4>计算路径</h4>
    <p>
      接下来做一个函数来计算出样式，clip-path 中的 path 模式和 svg 中的 path
      是一样的，我们主要用到4个命令：M 移动，L 绘制直线，A 绘制弧形，Z 关闭路径。
    </p>
    <p>
      <img
    ...
    <h4>添加交互效果</h4>
    <p>
      到这里就和之前使用背景图渐变实现的效果一样了，但是每一个扇区都是一个 dom
      元素，这意味着我们可以给它加样式或加事件来实现动态效果。那么继续改造一下，
      让鼠标悬停在扇区的时候突出扇区，并显示对应的标题。在每 .sector 的后面加一个兄弟元素 .title，当
      .sector 有鼠标悬停的时候 title 才在右侧显示出来。此外，再给 .sector
      增加一个鼠标悬停放大的效果。
    </p>
    <CodeHighlight
      code="
    ...
    <p>
      ...
      的话除了绘制状态外还需要实现元素边缘判定和事件绑定机制，而这些 dom
      本来就有的，有些交互不需要写 js 直接 css 就可以实现，你只管裁剪出需要的形状就好。但是，path
...
.pie {
  ...
  height: 250px;
  border-radius: 50%;
  background: conic-gradient(blue 36deg, green 36deg 170deg, cadetblue 170deg);
}
</style>
