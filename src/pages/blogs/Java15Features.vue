<template>
  <blog-layout>
    ...
    <p>
      TreeMap 中对这些方法进行了覆写：putIfAbsent、computeIfAbsent、computeIfPresent、compute、 和merge 。
      新覆写的这些方法相比 Map 接口中的默认方法实现提升了性能。
    </p>
    <h3>JMX 支持配置第三个端口</h3>
    ...
    <p>新的参数 -r &lt;port&gt; 添加到了 jstatd 用于指定 RMI 商品号，如果没有指定，将会使用一个随机端口号。</p>
    ...
    <ul>
      ...
      <li>--registryport &lt;port&gt; 用于指定 RMI 注册端口，未指定情况下使用系统属性，系统属性也没有设置则使用 1099。</li>
    ...
    <p>SunJCE provider 已得到增强，以支持 HmacSHA3-224、HmacSHA3-256、HmacSHA3-384 和 HmacSHA3-512。</p>
    <h3>新的系统属性用于配置 TLS 签名模式</h3>
    <p>新增了 jdk.tls.client.SignatureSchemes 和 jdk.tls.server.SignatureSchemes 分别用于客户端和服务器端配置 TLS签名模式。</p>
    ...
    <p>
      现在已经支持了 canonicalize 选项，如果此标志设置为 true，则对 KDC（Key Distribution Center 密钥分发中心）
    ...
    <ul>
      ...
      <li>废弃偏向锁和偏向锁相关的标记，偏向锁可能会影响存在大量无竞争同步的应用的性能，这个版本默认被禁用</li>
      <li>默认禁用 SunEC 的本地实现</li>
      <li>ContentSigner 和 ContentSignerParameters 之前已经废弃，这个版本打上 forRemoval=true 标记</li>
    </ul>
  </blog-layout>
</template>
