...
export function calculate(
  ...
  for (const item of suffixExpress) {
    ...
    if (item.notation === '+') {
      const sum = parseFloat(
        new BigNumber(num2.num).plus(new BigNumber(num1.num)).toFormat(4, roundingMode, fmt)
      )
      const plusItem = mergeNumItem(num1, num2, sum)
      numStack.push(plusItem)
      result.stages.push(
        buildStage(lastStage, {
          result: sum,
      ...
      const difference = parseFloat(
        new BigNumber(num2.num).minus(new BigNumber(num1.num)).toFormat(4, roundingMode, fmt)
      )
      ...
      const product = parseFloat(
        new BigNumber(num2.num).times(new BigNumber(num1.num)).toFormat(4, roundingMode, fmt)
      )
      ...
      const quotient = parseFloat(
        new BigNumber(num2.num).dividedBy(new BigNumber(num1.num)).toFormat(4, roundingMode, fmt)
      )
  ...
  if (numStack.length === 1) {
    result.result = parseFloat(
      new BigNumber(numStack[0].num).toFormat(precision, roundingMode, fmt)
    )
    return result
  }
