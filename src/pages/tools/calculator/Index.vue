<template>
  <layout>
    <div class="container-xxl py-3">
      <h1 class="pb-3 mb-2"><i class="fas fa-calculator me-2"></i>在线计算器</h1>
      <p class="lead mb-5">整式计算，自定义舍入模式和小数位，运算过程演示，结果金额大写转换，精准错误提示，本地历史记录存储</p>
      <div class="row mb-3 g-3">
        <div class="col-md-8">
          <label class="form-label">
            输入计算内容 （如<a href="javascript:" @click="inputExpress('1+2-3*4/(2+4)*50%')">1+2-3*4/(2+4)*50%</a>）
          </label>
          <textarea v-model="express" placeholder="仅支持一万亿以内数字且小数不超过4位，输入 * 等同于 x，输入 / 等同于 ÷，最长256个字符"
            class="form-control d-block" ref="textarea" rows="5" autofocus style="resize: vertical"
            @input="updateCursor($event)" @blur="updateCursor($event)" maxlength="256"></textarea>
        </div>
        <div class="col-md-4">
      ...
      <div v-if="result" class="mb-3 bg-light p-2">
        <span class="user-select-all">{{ result.express }}</span>=<span class="user-select-all text-primary">{{
            result.result
        }}</span>
        <span v-if="amountCapital"> / <span class="user-select-all text-primary">{{ amountCapital }}</span></span>
      </div>
      <p>
        <button type="button" class="btn btn-outline-secondary me-2 mb-2" @click="exec">
          <i class="fas fa-calculator margin-right-xs"></i>
          开始计算
        </button>
        <template v-if="result">
          <button type="button" class="btn btn-outline-secondary me-2 mb-2" @click="convertAmountCapital">
            <i class="fas fa-dollar-sign me-2"></i>转大写
          </button>
          <button type="button" class="btn btn-outline-secondary mb-2" @click="showStages">
            <i class="fas fa-list-ol me-2"></i>查看运算过程
          </button>
        </template>
        <span v-if="errMsg" class="text-danger">{{ errMsg }}</span>
      </p>
      <div class="bg-light p-2 mb-3"><i class="fas fa-keyboard me-2"></i>快速输入</div>
      <div class="row g-3 mb-3">
        <div class="col-3 col-lg-1">
          <button class="btn btn-outline-secondary d-block w-100 btn-sm" type="button" @click="inputNotation('+')">
            + 加
          </button>
        </div>
        <div class="col-3 col-lg-1">
        ...
        <div class="col-3 col-lg-1">
          <button class="btn btn-outline-secondary d-block w-100 btn-sm" type="button" @click="inputNotation('x')">
            x 乘
          </button>
        </div>
        <div class="col-3 col-lg-1">
      ...
      <ul class="list-group list-group-flush">
        <li v-for="(item, index) in histories" class="list-group-item">
          <div class="d-flex">
            ...
            <div class="flex-grow-0 flex-shrink-0">
              <a href="javascript:" @click="deleteHis(index)" class="link-danger">
        ...
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title"><i class="fas fa-list-ol me-2"></i>运算步骤</h5>
            ...
            <div class="modal-body p-0">
              <ul class="list-group list-group-flush">
                <li v-for="(stage, index) in stages"
                  ...
                  <div>
                    <div v-for="frag in stage" :key="frag.content" :class="{ 'text-danger': frag.highlight }"
                      class="d-inline">
                      {{ frag.content }}
                    </div>
                  </div>
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-secondary mx-auto" data-bs-dismiss="modal">关闭步骤演示</button>
            </div>
          </div>
        </div>
      </div>
