...
export function amountCapitalConvert(amount: number): string {
  if (amount === 0) {
    return '零元整'
  }
  if (amount < 0) {
  ...
  // 处理完小数,处理整数部分
  const positiveIntegerStr: string =
    amountStr.indexOf('.') === -1 ? amountStr : amountStr.substring(0, amountStr.indexOf('.'))
  // 整数从右往左开始
  // 处理万以内,从右开始四位以内
